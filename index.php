...
.brand-lockup{display:inline-flex;flex:0 0 auto;align-items:center;gap:12px;min-width:0}
.brand-logo{display:block;flex:0 0 auto;width:54px;height:54px;object-fit:contain;image-rendering:pixelated}
...
.submission-history-modal{width:min(900px,100%)}
.submission-history-list{display:grid;gap:14px;margin-top:18px}
.submission-round{border:1px solid var(--line);border-left:4px solid var(--green);border-radius:5px;background:var(--surface-soft);padding:15px}
.submission-round.REMOVE{border-left-color:var(--red)}
.submission-round-head{display:flex;align-items:flex-start;justify-content:space-between;gap:14px;margin-bottom:11px}
.submission-round-head h3{margin:2px 0 0;font-size:16px}
.submission-round-meta{display:flex;gap:8px;flex-wrap:wrap;color:var(--muted);font-size:11px}
.submission-round-total{font-size:18px;font-weight:850;white-space:nowrap;font-variant-numeric:tabular-nums}
.submission-history-modal details{border-top:1px solid var(--line);padding-top:10px}
.submission-history-modal summary{cursor:pointer;color:var(--green-dark);font-size:12px;font-weight:800}
.submission-history-modal .table-wrap{margin-top:10px}
.submission-history-modal .history-pagination{margin-top:18px}
.submission-history-filters{display:grid;grid-template-columns:minmax(150px,1.4fr) repeat(4,minmax(120px,1fr));gap:10px;align-items:end;margin-top:18px;padding:13px;background:var(--surface-soft);border:1px solid var(--line);border-radius:5px}
.submission-history-filters label{margin:0 0 5px}
.submission-history-filters input,.submission-history-filters select{min-width:0;padding:8px 9px}
.submission-history-tools{display:flex;gap:8px;align-items:center;margin-top:10px;flex-wrap:wrap}
.submission-history-tools .secondary{text-decoration:none}
.access-log-modal{width:min(860px,100%)}
.access-log-modal .table-wrap{margin-top:18px}
.access-log-table{table-layout:fixed}
.access-log-table th:nth-child(1){width:28%}
.access-log-table th:nth-child(2){width:24%}
.access-log-table th:nth-child(3){width:28%}
.access-log-table th:nth-child(4){width:20%}
.access-log-table td{white-space:normal;vertical-align:middle}
.access-log-time{font-variant-numeric:tabular-nums;color:#405a4e}
.access-log-ip{display:inline-block;max-width:100%;padding:4px 7px;border:1px solid #d4dfd7;border-radius:3px;background:#f3f7f4;color:var(--green-dark);font-family:ui-monospace,SFMono-Regular,Consolas,"Liberation Mono",monospace;font-size:12px;font-variant-numeric:tabular-nums;overflow-wrap:anywhere}
...
@media(max-width:820px){
  ...
  .access-log-table{table-layout:auto;font-size:12px}
  .access-log-table th,.access-log-table td{padding:10px 6px}
...
@media(max-width:640px){
  ...
  .brand-lockup{width:100%;gap:10px;margin-bottom:13px}
  .brand-logo{width:44px;height:44px}
  .brand{margin-bottom:0;font-size:25px}
...
const status=v=>v==='APPROVED'&&state.user?.uses_user_view?'':'<span class="badge '+esc(v)+'">'+esc(v)+'</span>';
  ...
  function layout(content){document.querySelector('#app').innerHTML='<header class="topbar"><div class="brand-lockup"><img class="brand-logo" src="/assets/beacon-logo.png" alt="" aria-hidden="true" width="128" height="128" decoding="async"><div class="brand">BEACON TEAM<small>Minecraft inventory control</small></div></div><div id="account"></div></header>'+content;const contentNode=document.querySelector('#content');if(contentNode)new MutationObserver(decorateContributors).observe(contentNode,{childList:true,subtree:true});}
...
async function openSubmissionHistory(page=1,filters=null){const opener=document.activeElement;let modal=document.querySelector('[data-submission-history]');if(!modal){modal=document.createElement('div');modal.className='modal-backdrop';modal.dataset.submissionHistory='1';modal.innerHTML='<section class="modal submission-history-modal" role="dialog" aria-modal="true" aria-labelledby="submissionHistoryTitle"><div class="modal-head"><div><div class="eyebrow">PRIVATE ADMIN HISTORY</div><h2 id="submissionHistoryTitle">ประวัติการอัปกรงเป็นรอบ</h2></div><button type="button" class="secondary" data-close-submission-history>ปิด</button></div><form class="submission-history-filters"><div><label>ชื่อผู้ใช้</label><input name="username" maxlength="80" placeholder="ค้นหาชื่อ"></div><div><label>ประเภท</label><select name="action"><option value="">ทั้งหมด</option><option value="ADD">เพิ่มกรง</option><option value="REMOVE">ถอนกรง</option></select></div><div><label>สถานะ</label><select name="status"><option value="">ทั้งหมด</option><option value="APPROVED">APPROVED</option><option value="PENDING_REVIEW">PENDING REVIEW</option><option value="BLOCKED">BLOCKED</option><option value="FLAGGED">FLAGGED</option><option value="REJECTED">REJECTED</option></select></div><div><label>ตั้งแต่วันที่</label><input name="date_from" type="date"></div><div><label>ถึงวันที่</label><input name="date_to" type="date"></div></form><div class="submission-history-tools"><button type="button" class="primary" data-apply-submission-filter>ค้นหา</button><button type="button" class="secondary" data-reset-submission-filter>ล้างตัวกรอง</button><button type="button" class="secondary" data-export-submission-history>ดาวน์โหลด CSV</button></div><div data-submission-history-content></div></section>';document.body.appendChild(modal);const close=()=>{modal.remove();opener?.focus?.()};modal.querySelector('[data-close-submission-history]').onclick=close;modal.onclick=event=>{if(event.target===modal)close()};modal.onkeydown=event=>{if(event.key==='Escape')close()};const form=modal.querySelector('.submission-history-filters');const readFilters=()=>Object.fromEntries(new FormData(form));form.onsubmit=event=>{event.preventDefault();openSubmissionHistory(1,readFilters())};modal.querySelector('[data-apply-submission-filter]').onclick=()=>openSubmissionHistory(1,readFilters());modal.querySelector('[data-reset-submission-filter]').onclick=()=>{form.reset();openSubmissionHistory(1,{})};modal.querySelector('[data-export-submission-history]').onclick=()=>{const query=new URLSearchParams({route:'admin/submission-history/export'});Object.entries(readFilters()).forEach(([key,value])=>{if(value)query.set(key,value)});location.assign('api.php?'+query.toString())}}const form=modal.querySelector('.submission-history-filters');if(filters===null)filters=Object.fromEntries(new FormData(form));const content=modal.querySelector('[data-submission-history-content]');content.innerHTML='<div class="user-inventory-loading" role="status">กำลังโหลดประวัติ</div>';try{const result=await api('admin/submission-history/'+page,{},filters);const rounds=result.batches.map((batch,index)=>{const rows=batch.items.map((item,itemIndex)=>'<tr><td>'+esc(itemIndex+1)+'</td><td>'+esc(item.cage_type)+'</td><td><strong>'+esc(item.quantity)+'</strong></td><td>'+status(item.status)+'</td></tr>').join('');return '<article class="submission-round '+esc(batch.action)+'"><div class="submission-round-head"><div><div class="submission-round-meta"><span>รอบที่ '+esc(((result.page-1)*result.per_page)+index+1)+'</span><span>'+displayTime(batch.created_at)+'</span><span>'+status(batch.status)+'</span></div><h3>'+esc(batch.username)+' · <span class="'+(batch.action==='ADD'?'quantity-add':'quantity-remove')+'">'+esc(batch.action==='ADD'?'เพิ่มกรง':'ถอนกรง')+'</span></h3></div><div class="submission-round-total">'+esc(batch.total_quantity)+' กรง</div></div><details><summary>ดู '+esc(batch.item_count)+' รายการในรอบนี้</summary><div class="table-wrap"><table><thead><tr><th>#</th><th>ชนิดกรง</th><th>จำนวน</th><th>สถานะ</th></tr></thead><tbody>'+rows+'</tbody></table></div></details></article>'}).join('');content.innerHTML=(rounds?'<div class="submission-history-list">'+rounds+'</div>':'<div class="empty-state" style="margin-top:18px">ไม่พบประวัติตามตัวกรองที่เลือก</div>')+'<div class="history-pagination"><button type="button" class="secondary" data-submission-previous '+(result.has_previous?'':'disabled')+'>ก่อนหน้า</button><span class="history-pagination-status">หน้า '+esc(result.page)+' · '+esc(result.batches.length)+' รอบ</span><button type="button" class="secondary" data-submission-next '+(result.has_next?'':'disabled')+'>ถัดไป</button></div>';content.querySelector('[data-submission-previous]').onclick=()=>openSubmissionHistory(Math.max(1,result.page-1),filters);content.querySelector('[data-submission-next]').onclick=()=>openSubmissionHistory(result.page+1,filters)}catch(error){content.innerHTML='<div class="user-inventory-empty"><strong>โหลดประวัติไม่สำเร็จ</strong><br>'+esc(error.message)+'</div>'}}
function accessLogCountryName(value){const code=String(value??'').trim().toUpperCase();if(!/^[A-Z]{2}$/.test(code))return 'ไม่ทราบ';try{return new Intl.DisplayNames(['th'],{type:'region'}).of(code)||code}catch(error){return code}}
async function openAccessLog(initialPage=1){
  if(!state.user?.is_primary_admin)return;
  const opener=document.activeElement;
  const modal=document.createElement('div');
  modal.className='modal-backdrop';
  modal.dataset.accessLog='1';
  modal.innerHTML='<section class="modal access-log-modal" role="dialog" aria-modal="true" aria-labelledby="accessLogTitle"><div class="modal-head"><div><div class="eyebrow">SECURITY ACCESS LEDGER</div><h2 id="accessLogTitle">ผู้เข้าใช้งาน</h2></div><button type="button" class="secondary" data-close-access-log>ปิด</button></div><div class="metric">ประวัติการเข้าสู่ระบบที่สำเร็จ แสดงครั้งละ 25 รายการ</div><div class="access-log-state" data-access-log-content aria-live="polite"></div></section>';
  document.body.appendChild(modal);
  const content=modal.querySelector('[data-access-log-content]');
  const close=()=>{modal.remove();opener?.focus?.()};
  modal.querySelector('[data-close-access-log]').onclick=close;
  modal.onclick=event=>{if(event.target===modal)close()};
  modal.onkeydown=event=>{if(event.key==='Escape')close()};
  const loadPage=async page=>{
    content.innerHTML='<div class="user-inventory-loading" role="status">กำลังโหลดประวัติผู้เข้าใช้งาน</div>';
    try{
      const result=await api('admin/access-log/'+page);
      const entries=Array.isArray(result.entries)?result.entries:[];
      const rows=entries.map(entry=>'<tr><td class="access-log-time">'+displayTime(entry.created_at)+'</td><td><strong>'+esc(entry.username)+'</strong></td><td><span class="access-log-ip">'+esc(entry.ip_address||'ไม่ทราบ')+'</span></td><td><span class="access-log-country" title="'+esc(String(entry.country_code??'').toUpperCase())+'">'+esc(accessLogCountryName(entry.country_code))+'</span></td></tr>').join('');
      content.innerHTML=(rows?'<div class="table-wrap"><table class="access-log-table"><thead><tr><th>เวลา</th><th>ชื่อผู้ใช้</th><th>IP Address</th><th>ประเทศ</th></tr></thead><tbody>'+rows+'</tbody></table></div>':'<div class="empty-state">ยังไม่มีประวัติการเข้าสู่ระบบ</div>')+'<div class="history-pagination"><button type="button" class="secondary" data-access-previous '+(result.has_previous?'':'disabled')+'>ก่อนหน้า</button><span class="history-pagination-status">หน้า '+esc(result.page)+' · '+esc(entries.length)+' รายการ</span><button type="button" class="secondary" data-access-next '+(result.has_next?'':'disabled')+'>ถัดไป</button></div>';
      content.querySelector('[data-access-previous]').onclick=()=>loadPage(Math.max(1,Number(result.page)-1));
      content.querySelector('[data-access-next]').onclick=()=>loadPage(Number(result.page)+1);
    }catch(error){
      content.innerHTML='<div class="user-inventory-empty"><strong>โหลดประวัติผู้เข้าใช้งานไม่สำเร็จ</strong><br>'+esc(error.message)+'<div style="margin-top:12px"><button type="button" class="secondary" data-retry-access-log>ลองใหม่</button></div></div>';
      content.querySelector('[data-retry-access-log]').onclick=()=>loadPage(page);
    }
  };
  modal.querySelector('[data-close-access-log]').focus();
...
renderApp=async function(){
  ...
  if(adminPage){
    ...
    if(document.querySelector('#submissionHistory'))document.querySelector('#submissionHistory').onclick=()=>openSubmissionHistory(1);
    if(document.querySelector('#accessLog'))document.querySelector('#accessLog').onclick=()=>openAccessLog(1);
